...
import { callGetTopicsByStatus, callUpdateTopicOutlineCoucilBulk, callUpdateTopicStatusBulk } from "../../../../../../services/api"
import { Document, Page, pdfjs } from 'react-pdf';
import { Buffer } from 'buffer';



const DrawerAssignTopic = (props) => {
    const { openDrawerAssignTopic, setOpenDrawerAssignTopic, reload, setReload, coucilTopic, isReport, setIsReport } = props
    ...
    useEffect(() => {
        const GetTopicByStatus = async () => {
            const res = await callGetTopicsByStatus(5)
            console.log('cc', res.data.payload)
            if (res) {
    ...
    const onclose = () => {
        setOpenDrawerAssignTopic(false)
        setReload(!reload)
    }
    ...
    const setTopicCoucil = async () => {
        ...
        const resOutlineCoucil = await callUpdateTopicOutlineCoucilBulk(arrayKey, coucilTopic?.id)
        const res = await callUpdateTopicStatusBulk(arrayKey, 6)
    ...
    return (
        <div>
            <Drawer title="Danh sách đề tài đủ điều kiện báo cáo đề cương" placement="right" onClose={onclose} open={openDrawerAssignTopic} width={1000} bordered={true}>
...
export default DrawerAssignTopic